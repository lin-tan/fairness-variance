...
def main():
    ...
    #'''
    bias_log = Path('./logs', 'bias_result.txt')
    with open(str(bias_log), 'r') as f:
        dataset_leakage = f.readline()
    dataset_leakage = float(dataset_leakage.split(':')[1].strip())
    #'''
    ...
    for feature_type in acc_list.keys():
        ...
        model_leakage = np.mean(np.array(acc_list[feature_type]))
        bias_amp = model_leakage - dataset_leakage
        with open(str(bias_log), 'a') as f:
            f.write('\n')
            f.write('{0} Lambda_M(F1): {1}'.format(feature_type, model_leakage))
            f.write('\n')
            f.write('{0} bias amplification(\Delta): {1}'.format(feature_type, bias_amp))
