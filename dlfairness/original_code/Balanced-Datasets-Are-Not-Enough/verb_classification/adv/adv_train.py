...
def main():
    ...
    parser.add_argument('--variance_logging', action='store_true')
    ...
    if args.resume:
        if os.path.isfile(os.path.join(args.save_dir, 'checkpoint.pth.tar')):
            ...
            best_mAP_man = checkpoint['best_mAP_man']
            best_mAP_woman = checkpoint['best_mAP_woman']
    ...
    for epoch in range(args.start_epoch, args.num_epochs + 1):
        ...
        current_performance, cur_mAP, cur_mAP_man, cur_mAP_woman = test(args, epoch, model, criterion, val_loader, \
                val_logger, logging = True)
        is_best = current_performance > best_performance
        best_performance = max(current_performance, best_performance)
        if is_best:
            best_mAP = cur_mAP
            best_mAP_man = cur_mAP_man
            best_mAP_woman = cur_mAP_woman
        model_state = {
            'epoch': epoch + 1,
            'state_dict': model.state_dict(),
            'best_mAP' : best_mAP,
            'best_mAP_man': best_mAP_man,
            'best_mAP_woman': best_mAP_woman,
    ...
    #'''
    # Write result log
    if args.variance_logging:
        variance_analysis_log = Path('./logs', 'test_result.txt')
        with open(str(variance_analysis_log), 'w') as f:
            f.write("F1 Score: " + str(best_performance))
            f.write('\n')
            f.write('mAP: '+ str(best_mAP))
            f.write('\n')
            f.write('mAP Men: ' + str(best_mAP_man))
            f.write('\n')
            f.write('mAP_Women: '+ str(best_mAP_woman))
    #'''
    ...
    for batch_idx, (images, targets, genders, image_ids) in enumerate(t):
        ...
        if args.batch_balanced:
            ...
            if selected_num < args.batch_size / 2:
                ...
                selected_num = int(args.batch_size / 2)
...
def test(args, epoch, model, criterion, val_loader, val_logger, print_every=10000, logging=True):
    ...
    return task_f1_score, meanAP, meanAP_man, meanAP_woman
