...
def run_baseline(args):
    ...
    preds, ground_truths, sens = classifier.predict_dataset(test_data, device=device) ## VARIANCE_PROJECT
    #'''
    Path(args.save_dir).mkdir(exist_ok=True, parents=True)
    p = Path(args.save_dir, 'preds.pth')
    torch.save(preds, str(p))
    p = Path(args.save_dir, 'ground_truths.pth')
    torch.save(ground_truths, str(p))
    p = Path(args.save_dir, 'protected.pth')
    torch.save(sens, str(p))
    #'''
    ...
    #'''
    p = Path(args.save_dir, 'test_result.txt')
    with open(p, 'w') as f:
        f.write('Accuracy: ' + str(metrics['Accuracy']))
    p = Path(args.save_dir, 'bias_result.txt')
    with open(p, 'w') as f:
        f.write('DP: ' + str(metrics['PPV_sens_0.0-sens_1.0']))
    #'''
