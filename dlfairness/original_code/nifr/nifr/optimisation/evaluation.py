...
def evaluate(
    ...
    if args.dataset in ("cmnist", "celeba", "ssrp", "genfaces"):
        ...
        if save_to_csv is not None:
            print("### Variance logging Store Raw")
            p = Path(save_to_csv, 'preds.pth')
            torch.save(preds, str(p))
            p = Path(save_to_csv, 'ground_truths.pth')
            torch.save(actual, str(p))
            p = Path(save_to_csv, 'protected.pth')
    ...
    #'''
    if save_to_csv is not None:
        p = Path(save_to_csv, 'test_result.txt')
        with open(p, 'w') as f:
            f.write('Accuracy: ' + str(metrics['Accuracy']))
        p = Path(save_to_csv, 'bias_result.txt')
        with open(p, 'w') as f:
